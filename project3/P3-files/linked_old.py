...
BLOCK_SIZE = 0
directory = [0] * available_blocks
DT = []
FAT = [-2] * available_blocks
pointer_size = 4
...
# returns random int value between 0 and 9 million.
def random_directory_value():
    # this method is to randomly generate values for directory when a block is full
    return randrange(9000000)
...
def recursive_extend(extension, counter, prev_index, list_index):
    ...
    for index, block in enumerate(directory[list_index+1:], start=list_index+1):
        if block == 0:
            ...
            directory[index] = random_directory_value()
            return recursive_extend(extension - 1, counter + 1, index, index)


# returns number of blocks of the file that is used to subtract from available_blocks after completion,
...
def create_table(file_id, file_length):
    ...
    num_block = math.ceil((file_length + pointer_size * num_block) / BLOCK_SIZE)
    # num_block calculated again with pointer * num_block size added.

    if num_block <= available_blocks: # if here is enough space to create that file
        starting_index = 0
        prev_index = -1 # FAT[prev_index] will be set to -1 at the end, to indicate the end of the file
        counter = 0 # holds the empty block number
    ...
    if extension <= available_blocks:
        ...
        for index, file in enumerate(DT):
            if file_id == file["file_id"]:
                ...
                total_bytes = file["file_length"] + block_num * pointer_size
                ...
                recursive_extend(extension, 0, last_block_index, -1) # updates directory and FAT
                total_bytes_final = file["file_length"] + (BLOCK_SIZE * extension) + (extension * pointer_size)
...
def shrink(file_id, shrinking):
    ...
    for index, file in enumerate(DT):
        if file_id == file["file_id"]:
            ...
            total_bytes = file["file_length"] + n_block * pointer_size
            ...
            if n_block > shrinking:
                ...
                prev_index = starting_index
                last_block_index = 0
                while prev_index != -1: # for going to the end of the file
                    last_block_index = prev_index
                    prev_index = FAT[prev_index]
                pointer_index = last_block_index # when we reach the end pointer_index = -1

                while (shrinking > 0) and (pointer_index != -1): # removing process until shrinking == 0
                    shrinking -= 1
                    FAT[pointer_index] = -2
                ...
                total_bytes_final = file["file_length"] - (BLOCK_SIZE * original_shrinking) - \
                                    (original_shrinking * pointer_size)
            ...
            for line in a_file:
                ...
                if operation == 'c':
                    ...
                    if extension_block != -1:
                        available_blocks -= extension_block
                        extend_success += 1
                    else:
                        extend_reject += 1
                elif operation == 's':
